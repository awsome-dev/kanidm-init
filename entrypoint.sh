...
# ==============================================================================
# このスクリプトは、Kanidm サーバーの起動前に初期化ツール (kanidm-init) を実行し、
...
# 1. 初期設定プロセスの実行
# kanidm-init は内部で以下の「冪等性の判定」と「フェーズ 1~3」を順次実行する：
...
if [ -f "/data/setup.toml" ]; then
    echo "Executing kanidm-init with /data/setup.toml..."
    /sbin/kanidm-init \
        --config-path /data/server.toml \
        --setup-toml /data/setup.toml
    echo "kanidm-init execution completed."
else
